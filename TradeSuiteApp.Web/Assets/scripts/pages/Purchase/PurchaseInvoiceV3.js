...
purchase_invoice = {
    init: function () {
        ...
        $('#tabs-purchase-invoice-create').on('change.uk.tab', function (event, active_item, previous_item) {
    ...
    bind_events: function () {
        ...
        $("body").on('click', ".btnpiprint", self.Purchaseinvoicedetailpage);
        ...
        $('body').on('keyup', '#tbl-item-details tbody tr .DiscountPercentage,.DiscountAmount, .VATPercentage, .VATAmount', self.calculate_invoice_total);
        $('body').on('keyup', '#txtDiscountPercent,#txtDiscount,#txtVatPercentage,#txtVATAmount,#txtSupplierOtherCharges,#txtSuppDocAmount,#txtSuppShipAmount,#txtPackingForwarding,#txtTotalFreight,#txtLocalCustomsDuty,#txtLocalFreight,#txtLocalMiscCharge,#txtLocalOtherCharges,#SGSTAmt,#CGSTAmt,#IGSTAmt', self.Calculate_invoice_Totals);
        $('#Discount, #SGSTAmt , #CGSTAmt , #IGSTAmt').on('keyup', self.calculate_total);
        $('body').on('click', '#tbl-item-details tbody tr td.showitemhistory', self.showitemhistory);
    },
    ...
    get_grn_autocomplete: function (release) {
        $.ajax({
            url: '/Purchase/GRN/GetUnProcessedGRNAutoCompleteV3',
            data: {
    ...
    get_grn_items_for_allopathy_autocomplete: function (item) {
        ...
        $.ajax({
            url: '/Purchase/PurchaseInvoice/GRNItemsForPurchaseInvoiceV3',
            data: {
                grnIDS: grn_ids,
    ...
    on_change_gst: function () {
        ...
        $(row).find('.NetAmount').val(totalamount.roundToCustom())
    ...
    claculate_grid_total: function () {
        var self = purchase_invoice;
        var netamount = 0, gst, igst = 0, cgst = 0, sgst = 0, total = 0, discount = 0, grossamount = 0;
        var netdiscount = 0;
        var netSgstAmount = 0;
        var netCgstAmount = 0;
        var netIgstAmount = 0;
        ...
        $("#tbl-item-details tbody tr").each(function () {
            ...
            let discount = clean($(this).find('.DiscountAmount').val());
    ...
    getCalculatedTDSAmt: function (amt, percentage) {
        ...
        if (percentage != '') {
            return ((parseFloat(amt) / parseFloat(100)) * parseFloat(percentage)).roundToCustom();
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == "success") {
                    ...
                    setTimeout(function () {
                        window.location = "/Purchase/PurchaseInvoice/IndexV3";
                    }, 1000);
                } else {
    ...
    calculate_grid_total: function (row) {
       ...
       // Discount = Math.round(Discount);
        ...
        var grossamount = Rate * qty;
        var taxableamount = grossamount - Discount;
        var gstamount = igst * taxableamount / 100;
        var totalamount = gstamount + VATAmount + taxableamount;
        var discountpercentage = Discount / grossamount * 100;
        var VATPercentage = VATAmount / taxableamount * 100;
        if (IsGSTRegistered == "false") {
            IGSTAmt = 0.00;
            SGSTAmt = 0.00;
            CGSTAmt = 0.00;
            ...
            if (ShippingStateID != StateID) {
                IGSTAmt = igst * taxableamount / 100;
                SGSTAmt = 0.00;
                CGSTAmt = 0.00;
                IGSTPercent = igst;
                SGSTPercent = 0.00;
            ...
            //else if (item.SGSTAmt > 0 && item.CGSTAmt > 0 && item.IGSTAmt == 0)
                ...
                CGSTPercent = igst / 2;
                IGSTAmt = 0.00;
                SGSTAmt = SGSTPercent * taxableamount / 100;
                CGSTAmt = CGSTPercent * taxableamount / 100;
            }
        }
        ...
        $(row).find('.CGSTPercent').val(CGSTPercent);
        $(row).find('.IGSTPercent').val(IGSTPercent);
        $(row).find('.SGSTAmt').val(SGSTAmt.roundToCustom());
        $(row).find('.CGSTAmt').val(CGSTAmt.roundToCustom());
        $(row).find('.IGSTAmt').val(IGSTAmt.roundToCustom());
        $(row).find('.DiscountPercentage').val(discountpercentage.roundToCustom());
        $(row).find('.VATPercentage').val(VATPercentage.roundToCustom());
        $(row).find('.txtItemInvoiceValue').val(grossamount.roundToCustom());
    ...
    calculate_invoice_total: function () {
        ...
        var discountPercent = clean($currtr.find('.DiscountPercentage').val());
        var VATAmount = clean($currtr.find('.VATAmount').val());
        var VATPercentage = clean($currtr.find('.VATPercentage').val());
        var approvedvalue = clean($currtr.find('.itemApprovedValue ').text());
        var invValue = invQty * invRate;
        if (className.indexOf('DiscountPercentage') != -1) {
            cname = 'DiscountPercentage';
            discount = invValue * discountPercent / 100;
            $currtr.find('.DiscountAmount').val(discount.roundToCustom());
        } else if (className.indexOf('DiscountAmount') != -1) {
            cname = 'DiscountAmount';
            discountPercent = discount / invValue * 100;
            $currtr.find('.DiscountPercentage').val(discountPercent.roundToCustom());
        }
        var itemTaxable = invValue - discount;
        if (className.indexOf('VATPercentage') != -1) {
            cname = 'VATPercentage';
            VATAmount = itemTaxable * VATPercentage / 100;
            $currtr.find('.VATAmount').val(VATAmount.roundToCustom());
        } else if (className.indexOf('VATAmount') != -1) {
            cname = 'VATAmount';
            VATPercentage = VATAmount / itemTaxable * 100;
            $currtr.find('.VATPercentage').val(VATPercentage.roundToCustom());
        } else {
            VATAmount = itemTaxable * VATPercentage / 100;
            $currtr.find('.VATAmount').val(VATAmount.roundToCustom());
        }
        var netamount = invValue - discount + VATAmount;
        $currtr.find('.TaxableAmount').val(itemTaxable.roundToCustom());
        $currtr.find('.txtItemInvoiceValue').val(invValue.roundToCustom());
        $currtr.find('.NetAmount').val(netamount.roundToCustom());
        $currtr.find('.itemDiffValue').val((invValue - approvedvalue).roundToCustom());
        self.Calculate_invoice_Totals(cname);
        fh_item.resizeHeader();
    },
    Calculate_invoice_Totals: function (itemclassname = '') {
        var currVal = 0, totalInvoice = 0, discount = 0, discountPercent = 0, VatPercentage = 0, VATAmount = 0, grossamount = 0, row, netamount = 0, TaxableAmount = 0, roundoff = 0, discountper = 0, VATItemper = 0,
            LocalLandinngCost = 0;
        var SuppDocAmount = clean($("#txtSuppDocAmount").val());
        var SuppShipAmount = clean($("#txtSuppShipAmount").val());
        var PackingForwarding = clean($("#txtPackingForwarding").val());
        var SuppOthCharges = clean($("#txtSupplierOtherCharges").val());
        var TotalFreight = clean($("#txtTotalFreight").val());
        var LocalCustomsDuty = clean($("#txtLocalCustomsDuty").val());
        var LocalFreight = clean($("#txtLocalFreight").val());
        var LocalMiscCharge = clean($("#txtLocalMiscCharge").val());
        var LocalOtherCharges = clean($("#txtLocalOtherCharges").val()); 
        var Igst = clean($("#IGSTAmt").val());
        var Sgst = clean($("#SGSTAmt").val());
        var Cgst = clean($("#CGSTAmt").val());
        var className = $(this).attr('id');
        if (className !== undefined && className == 'txtDiscountPercent') {
            var GrossAmt = clean($("#txtGrossAmount").val());
            discountPercent = clean($("#txtDiscountPercent").val());
            discount = GrossAmt * discountPercent / 100;
            discountper = discountPercent;
        } else if (className !== undefined && className == 'txtDiscount') {
            var GrossAmt = clean($("#txtGrossAmount").val());
            discount = clean($("#txtDiscount").val());
            discountPercent = ((discount * 100) / GrossAmt);
            discountper = discountPercent;
        } else if (className !== undefined && className == 'txtVatPercentage') {
            var TaxableAmt = clean($("#txtGrossAmount").val()) - clean($("#txtDiscount").val());
            VatPercentage = clean($("#txtVatPercentage").val());
            VATAmount = TaxableAmt * VatPercentage / 100;
            VATItemper = VatPercentage;
        } else if (className !== undefined && className == 'txtVATAmount') {
            var TaxableAmt = clean($("#txtGrossAmount").val()) - clean($("#txtDiscount").val());
            VATAmount = clean($("#txtVATAmount").val());
            VatPercentage = ((VATAmount * 100) / TaxableAmt);
            VATItemper = VatPercentage;
        }
        discount = 0;
        discountPercent = 0;
        ...
        $('#item-details-tblContainer table tr td .chkItem:checked').parents('tr').find('.NetAmount').each(function () {
            ...
            var row = $(this).closest('tr');
            var itemgrossamount = clean($(row).find('.txtItemInvoiceValue').val());
            var itemdiscount = 0;
            var itemvatamount = 0;
            var itemNetAmount = 0;
            var itemTaxableAmount = 0;
            if (className !== undefined && (className == 'txtDiscountPercent' || className == 'txtDiscount')) {
                itemdiscount = (itemgrossamount * discountper / 100);
                $(row).find('.DiscountPercentage').val(discountper.roundToCustom());
                $(row).find('.DiscountAmount').val(itemdiscount.roundToCustom());
            } else {
                itemdiscount = clean($(row).find('.DiscountAmount').val());
                var itemdiscountpercentage = ((itemdiscount * 100) / itemgrossamount);
                if (itemclassname !== undefined && itemclassname == 'DiscountPercentage') {

                } else if (itemclassname !== undefined && itemclassname == 'DiscountAmount') {
                    $(row).find('.DiscountPercentage').val(itemdiscountpercentage.roundToCustom());
                }

            }
            itemTaxableAmount = itemgrossamount - itemdiscount;
            if (className !== undefined && (className == 'txtVatPercentage' || className == 'txtVATAmount')) {
                itemvatamount = (itemTaxableAmount * VATItemper / 100);
                $(row).find('.VATPercentage').val(VATItemper.roundToCustom());
                $(row).find('.VATAmount').val(itemvatamount.roundToCustom());

            } else {
                itemvatamount = clean($(row).find('.VATAmount').val());
                var vatitempercentage = (itemvatamount / itemTaxableAmount * 100);
                if (itemclassname !== undefined && itemclassname == 'VATAmount') {
                    $(row).find('.VATPercentage').val(vatitempercentage.roundToCustom());
                } else if (itemclassname !== undefined && itemclassname == 'VATPercentage') {

                }

            }
            itemNetAmount = itemgrossamount - itemdiscount + itemvatamount;
            $(row).find('.NetAmount').val(itemNetAmount.roundToCustom());
            VATAmount += itemvatamount;
            netamount += itemNetAmount;
            discount += itemdiscount;
          //  discount = discount.roundToCustom();
            grossamount += itemgrossamount;
            TaxableAmount += itemTaxableAmount;
            VatPercentage = $(row).find('.VATPercentage').val();
        });

        $("#txtGrossAmount").val(grossamount);
        if (className !== undefined && className == 'txtDiscountPercent') {
            $("#txtDiscount").val(discount.roundToCustom());
        } else if (className !== undefined && className == 'txtDiscount') {
            discountPercent = discount / grossamount * 100;
            $("#txtDiscountPercent").val(discountPercent.roundToCustom());
        } else {
            $("#txtDiscount").val(discount.roundToCustom());
        ...
        if (className !== undefined && className == 'txtVatPercentage') {
            $("#txtVATAmount").val(VATAmount.roundToCustom());
        } else if (className !== undefined && className == 'txtVATAmount') {
            VatPercentage = VATAmount / TaxableAmount * 100;
            $("#txtVatPercentage").val(VatPercentage.roundToCustom());
        } else {
            $("#txtVATAmount").val(VATAmount.roundToCustom());
            $("#txtVatPercentage").val(VatPercentage);
        }
        var OtherChargesVATAmount = (SuppOthCharges + LocalOtherCharges) * VatPercentage / 100;

        netamount = grossamount - discount + Igst + Sgst + Cgst + VATAmount + SuppDocAmount + SuppShipAmount + PackingForwarding + SuppOthCharges + TotalFreight + OtherChargesVATAmount;
        LocalLandinngCost = LocalCustomsDuty + LocalFreight + LocalMiscCharge + LocalOtherCharges;
        $("#txtAmountPayable").val(netamount.roundToCustom());
        $("#txtLocalLandingCost").val(LocalLandinngCost);
        $("#OtherChargesVATAmount").val(OtherChargesVATAmount);
        //$("#txtDeductions").val(0);//roundoff
    },
    ...
    clear_items: function () {
        ...
        $('#txtDiscount').val('');
        $('#txtSupplierOtherCharges').val('');
        $('#txtSuppDocAmount').val('');
        $('#txtSuppShipAmount').val('');
        $('#txtTotalFreight').val('');
        $('#txtPackingForwarding').val('');
        $("#txtLocalCustomsDuty").val('');
        $("#txtLocalFreight").val('');
        $("#txtLocalMiscCharge").val('');
        $("#txtLocalOtherCharges").val('');
        $('#GRNNo').val('');
        $('#RoundOff').val(0);
        $('#SGSTAmt').val(0);
        $('#CGSTAmt').val(0);
    ...
    select_supplier: function () {
        ...
        gridcurrencyclass = app.change_decimalplaces($("#txtAmountPayable"), DecimalPlaces);
        app.change_decimalplaces($("#CurrencyExchangeRate"), DecimalPlaces);
        app.change_decimalplaces($("#txtDiscount"), DecimalPlaces);
        app.change_decimalplaces($("#txtSupplierOtherCharges"), DecimalPlaces);
        app.change_decimalplaces($("#txtTotalFreight"), DecimalPlaces);
        app.change_decimalplaces($("#txtGrossAmount"), DecimalPlaces);
        app.change_decimalplaces($("#txtSuppShipAmount"), DecimalPlaces);
        app.change_decimalplaces($("#txtPackingForwarding"), DecimalPlaces);
        app.change_decimalplaces($("#txtLocalCustomsDuty"), DecimalPlaces);
        app.change_decimalplaces($("#txtLocalFreight"), DecimalPlaces);
        app.change_decimalplaces($("#txtLocalMiscCharge"), DecimalPlaces);
        app.change_decimalplaces($("#txtLocalOtherCharges"), DecimalPlaces);
        //app.change_decimalplaces($("#txtDeductions"), DecimalPlaces);
        //app.change_decimalplaces($("#SGSTAmt"), DecimalPlaces);
        //app.change_decimalplaces($("#CGSTAmt"), DecimalPlaces);
        //app.change_decimalplaces($("#IGSTAmt"), DecimalPlaces);
    ...
    get_alopathy_grn: function () {
        var self = purchase_invoice;
        $.ajax({
            url: '/Purchase/GRN/GetUnProcessedGRNV3',
            data: {
                SupplierID: $("#SupplierID").val(),
                Hint: ""
            },
    ...
    get_grn_items_for_allopathy: function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                app.format($('#tbl-item-details'));
                setTimeout(function () { fh_item.resizeHeader(); }, 100);
                $("#txtSuppDocAmount").val(clean($(".hiddenSuppDocAmount").val()));
                $("#txtSupplierOtherCharges").val(clean($(".hiddenSuppOtherCharge").val()));
                $("#txtSuppShipAmount").val(clean($(".hiddenSuppShipAmount").val()));
                $("#txtTotalFreight").val(clean($(".hiddenSuppFreight").val()));
                $("#txtPackingForwarding").val(clean($(".hiddenPackingForwarding").val()));
                $("#txtLocalCustomsDuty").val(clean($(".hiddenLocalCustomsDuty").val()));
                $("#txtLocalFreight").val(clean($(".hiddenLocalFreight").val()));
                $("#txtLocalMiscCharge").val(clean($(".hiddenLocalMiscCharge").val()));
                $("#txtLocalOtherCharges").val(clean($(".hiddenLocalOtherCharges").val()));
                $('#tbl-item-details tbody tr').each(function () {
                    var row = $(this).closest('tr');
                    self.calculate_grid_total(row);
                });
                self.Calculate_invoice_Totals();
            }
        ...
        //self.get_grn_master(grn_ids);
    },

    get_grn_master: function (grn_ids) {
        var self = purchase_invoice;
        $.ajax({
            url: '/Purchase/PurchaseInvoice/GRNMasterForPurchaseInvoiceV3',
            data: {
                grnIDS: grn_ids
            },
            dataType: "json",
            ...
            var list_table = $list.dataTable({
                ...
                "aoColumns": [
...

                    {
                        ...
                        "render": function (data, type, row, meta) {
                            /* if (row.Status === 'draft') {*/
                            return "<button class='md-btn md-btn-primary md-btn-block md-btn-wave-light waves-effect waves-button waves-light btnpiprint' value = '" + row.ID + "'>Print</button>"
                        }
                    }
                ...
                "drawCallback": function () {
                    $list.find('tbody td:not(.action)').on('click', function () {
                        ...
                        app.load_content("/Purchase/PurchaseInvoice/GenerateDetailsV3/" + Id);
                    });
                },
            });
    ...
    Purchaseinvoicedetailpage: function () {
        var self = purchase_invoice;
...
function GetPurchaseInvoiceSaveObj() {
    ...
    root = {
        ...
        Discount: clean($('#txtDiscount').val()),
        TaxableAmount: clean($('#txtTaxableAmount').val()),
...
function GetPurchaseInvoiceData() {
    ...
    root = {
        ...
        SupplierOtherCharges: clean($('#txtSupplierOtherCharges').val()),
        SuppDocAmount: clean($('#txtSuppDocAmount').val()),
        SuppShipAmount: clean($('#txtSuppShipAmount').val()),
        FreightAmount: clean($('#txtTotalFreight').val()),
        PackingForwarding: clean($('#txtPackingForwarding').val()),
        LocalCustomsDuty: clean($("#txtLocalCustomsDuty").val()),
        LocalFreight : clean($("#txtLocalFreight").val()),
        LocalMiscCharge : clean($("#txtLocalMiscCharge").val()),
        LocalOtherCharges: clean($("#txtLocalOtherCharges").val()),
        LocalLandingCost: clean($("#txtLocalLandingCost").val()),
        ...
        OtherChargesVATAmount: clean($('#OtherChargesVATAmount').val()),
        //NetAmount: 0,           //
        //LocationID: 0
    };
    ...
    $('#item-details-tblContainer table#tbl-item-details tbody tr .chkItem:checked').each(function () {
        ...
        var secondaryInvoiceQty = clean($currRow.find('.txtSecondaryInvoiceQty').val());
        var secondaryOfferQty = clean($currRow.find('.SecondaryOfferQty').val());
        var secondaryRate = clean($currRow.find('.txtSecondaryRate').val());
        ...
        invoiceTransArr.push(
            {
                ...
                InvoiceRate: invRate,
                InvoiceValue: invValue,
                SecondaryInvoiceQty: secondaryInvoiceQty,
                SecondaryOfferQty: secondaryOfferQty,
                SecondaryRate: secondaryRate,
                SecondaryUnitSize: clean($currRow.find('.SecondaryUnitSize').val()),
                SecondaryUnit: $currRow.find('.SecondaryUnit').text().trim(),
                AcceptedQty: $currRow.find('.itemAcceptedQty').text().replace(/\,/g, ''),
                ApprovedQty: $currRow.find('.itemAprovedQty').text().replace(/\,/g, ''),
                ApprovedValue: $currRow.find('.itemApprovedValue').text().replace(/\,/g, ''),
                ...
                GrossAmount: clean($currRow.find('.txtItemInvoiceValue').val()),
                DiscountAmount: clean($currRow.find('.DiscountAmount').val()),
                DiscountPercentage: clean($currRow.find('.DiscountPercentage').val()),
                VATAmount: clean($currRow.find('.VATAmount').val()),
                VATPercentage: clean($currRow.find('.VATPercentage').val()),
                TaxableAmount: clean($currRow.find('.TaxableAmount').val()),
                NetAmount: clean($currRow.find('.NetAmount').val()),
                ItemName: $currRow.find('.ItemName').text().trim(),
                PartsNumber: $currRow.find('.PartsNumber').text().trim(),
                Remark: $currRow.find('.Remark').text().trim(),
                Model: $currRow.find('.Model').text().trim(),
                IsGST: clean($currRow.find('.IsGST').val()),
                IsVat: clean($currRow.find('.IsVat').val()),
                CurrencyID: clean($currRow.find('.CurrencyID').val()),
