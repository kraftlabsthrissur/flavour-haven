PatientDiagnosis.XrayTest = [];
PatientDiagnosis.XrayItemList = [];
PatientDiagnosis.XrayItemList2 = [];
        ...
        var list_table = $list.dataTable({
            ...
            "drawCallback": function () {
                $list.find('tbody td:not(.action)').on('click', function () {
                    ...
                    app.load_content("/AHCMS/PatientDiagnosis/CreateV5/?ID=" + PatientID + "&AppointmentScheduleItemID=" + ScheduleItemID + "&OPID=" + OPID + "&IsCompleted=" + IsCompleted + "&IsReferedIP=" + IsReferedIP + "&ReviewID=" + reviewID + "&IsWalkin=" + IsWalkin);
                });
            },
        });
...
PatientDiagnosis.Save = function () {
    ...
    data = self.get_data();
    $.ajax({
        url: '/AHCMS/PatientDiagnosis/SaveV5',
        data: data,
        dataType: "json",
        type: "POST",
        success: function (response) {
            if (response.Status == "success") {
                ...
                setTimeout(function () {
                    window.location = "/AHCMS/PatientDiagnosis/IndexV5";
                }, 1000);
            }
            else {
...
PatientDiagnosis.get_report = function () {
    var self = PatientDiagnosis;
    var PatientID = $('#PatientID').val();
    var Date = $('#Date option:selected').val();
    ...
    $.ajax({
        url: "/AHCMS/PatientDiagnosis/GetReportv5",
        data: {
            PatientID: PatientID,
            Date: Date,
...
PatientDiagnosis.add_report = function () {
    var self = PatientDiagnosis;
    self.error_count = 0;
    ...
    if ($("#RowIndex").val() > 0) {
        ...
        $(rowindex).find(".IsBeforeAdmission").val(IsBeforeAdmission);
...
PatientDiagnosis.get_previous_examination = function (ReviewID) {
    var self = PatientDiagnosis;
    var PatientID = $('#PatientID').val();
    $.ajax({
        url: "/AHCMS/PatientDiagnosis/PreviousExaminationV5",
        data: {
            PatientID: PatientID,
            ReviewID: ReviewID
        },
...
PatientDiagnosis.lablist = function () {
...
PatientDiagnosis.xraylist = function () {
    var $list = $('#xray-list');
...
PatientDiagnosis.get_xray_details_modal = function () {
    var self = PatientDiagnosis;
    var data = {};
...
PatientDiagnosis.xray_check = function () {
    var self = PatientDiagnosis
    var row = $(this).closest('tr');
    if ($(this).is(":checked")) {
...
PatientDiagnosis.add_xray_data_to_grid = function () {
    var self = PatientDiagnosis;
...
PatientDiagnosis.xray_delete_item = function () {
    var self = PatientDiagnosis;
...
PatientDiagnosis.get_XrayItems = function () {
    var self = PatientDiagnosis;
    var PatientID = $('#PatientID').val();
    ...
    var Date = $('#Date option:selected').val();
    $.ajax({
        url: "/AHCMS/PatientDiagnosis/XrayTest",
        dataType: "html",
        data: {
            PatientID: PatientID,
            OPID: OPID,
...
PatientDiagnosis.is_patient_history = function () {
    var self = PatientDiagnosis;
    var AppointmentProcessID = $('#AppointmentProcessID').val();
    var IsCompleted = $("#IsCompleted").val();
    var PatientID = $("#PatientID").val();
...
PatientDiagnosis.get_DashaVidhaPareekhsa = function () {
    var self = PatientDiagnosis;
    var PatientID = $('#PatientID').val();
    var AppointmentProcessID = $('#AppointmentProcessID').val();
    $.ajax({
        url: "/AHCMS/PatientDiagnosis/GetDashaVidhaPareekhsa",
        data: {
            PatientID: PatientID,
            AppointmentProcessID: AppointmentProcessID,
        },
