...
stockadjustment = {
            ...
            var list_table = $list.dataTable({
                ...
                "drawCallback": function () {
                    $list.find('tbody td:not(.action)').on('click', function () {
                        ...
                        app.load_content("/Stock/StockAdjustment/DetailsV3/" + Id);
                    });
                },
            });
    ...
    get_items: function (release) {
        ...
        $.ajax({
            url: '/Masters/Item/getStockAdjustmentItemForAutoCompleteV3',
            data: {
                term: $('#ItemName').val(),
    ...
    rules: {
...

        {
            elements: "#BatchType",
            rules: [
                { type: form.required, message: "Please choose Batch Type" },
                { type: form.non_zero, message: "Please choose Batch Type" },
    ...
    get_filter_item: function () {
        ...
        var self = stockadjustment;
        $.ajax({
            url: '/Stock/StockAdjustment/GetStockAdjustmentItems/',
            dataType: "json",
            data: {
            ...
            success: function (response) {
                ...
                $(response.Data).each(function (i, item) {
                    ...
                    var slno = (i + 1);
                    var Warehouse = $("#WarehouseID option:selected").text()
                    var DamageType = '<select class="md-input label-fixed DamageTypeID">' + stockadjustment.Build_Select(DamageTypeList) + '</select>'
                    var excessQty = item.PhysicalQty - item.CurrentQty;
                    var excessValue = excessQty * item.Rate;
                    content += '<tr>'
                        ...
                        + '<td class="ExpiryDate">' + item.ExpiryDateString + '</td>'
                        + '<td class="mask-production-qty currentqty ">' + item.CurrentQty + '</td>'
                        + '<td  >' + '<input type="text" value=" ' + item.PhysicalQty + '" class="md-input uk-text-right physicalqty mask-production-qty" disabled /> ' + '</td>'
                        + '<td class="ExcessQty mask-production-qty">' + excessQty + '</td>'
                        + '<td class="ExcessValue mask-production-qty">' + excessValue + '</td>'
                        + '<td  >' + '<input type="text" class="md-input uk-text Remark "  /> ' + '</td>'
                        + '<td >' + DamageType + '</td>'
                        + '<td>' + '<div class="uk-input-group"><button class="btnOpenBatchWithZeroQty" >+</button></div></td>'
                        + '</tr>';
    ...
    on_save: function () {
        ...
        var location = "/Stock/StockAdjustment/IndexV3";
        var url = '/Stock/StockAdjustment/SaveV3';

        if ($(this).hasClass("btnSavedraft")) {
            data.IsDraft = true;
            url = '/Stock/StockAdjustment/SaveAsDraftV3'
            self.error_count = self.validate_draft();
        } else {
            self.error_count = self.validate_submit();
            if ($(this).hasClass("btnSaveNew")) {
                location = "/Stock/StockAdjustment/CreateV3";
            }
        }
