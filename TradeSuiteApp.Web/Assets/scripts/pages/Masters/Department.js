Department = {
    ...
    list: function () {
        ...
        if ($list.length) {
            ...
            $('#department-list tbody tr').on('click', function () {
                var id = $(this).find('.ID').val();
                window.location = '/Masters/Department/Details/' + id;
            });
            altair_md.inputs();
    ...
    save: function () {
        ...
        $.ajax({
            url: '/Masters/Department/Save',
            data: modal,
            dataType: "json",
            type: "POST",
            success: function (data) {
                if (data.Status == "Success") {
                    app.show_notice("Department created successfully");
                    setTimeout(function () {
                        window.location = "/Masters/Department/Index";
                    }, 1000);
                } else {
    ...
    get_data: function () {
        var model = {
            ...
            Code: $("#Code").val(),
            Name: $("#Name").val(),
            DepartmentGroupID: $("#DepartmentGroupID").val(),
    ...
    rules: {
        on_submit: [
             ...
             {
                 elements: "#DepartmentGroupID",
                 rules: [
                     { type: form.required, message: "Department Group is Required" },
                 ]
             },
             {
                 elements: "#EndDate",
