...
Item = {
    ...
    Details: function () {
        var self = Item;
        fh_item = $("#tbl-invoice-details").FreezeHeader();
    ...
    list: function () {
        ...
        $list.on('click', 'tbody td', function () {
            ...
            window.location = '/Masters/Item/Details/' + Id;
        });
        if ($list.length) {
            $list.find('thead.search th').each(function () {
                var title = $(this).text().trim();
    ...
    stock_adjustment_item_list_alopathy: function () {
        var $list = $('#item-list');
        if ($list.length) {
            $list.find('thead.search th').each(function () {
                var title = $(this).text().trim();
            ...
            altair_md.inputs();
            var url = "/Masters/Item/GetItemsListForStockAdjustmentForAlopathy";
            ...
            var list_table = $list.dataTable({
                ...
                "ajax": {
                    ...
                    "data": function (data) {
                        data.params = [
                            { Key: "FromDate", Value: $('#FromDate').val() },
                            { Key: "ToDate", Value: $('#ToDate').val() }
    ...
    get_description: function (ItemID, Type) {
        var self = Item;
        $.ajax({
            url: '/Masters/Item/GetDescription',
            data: {
                ItemID: ItemID,
                Type: Type
            },
            ...
            success: function (data) {
                if (Type == "Purchase") {
                    ...
                    $('.Descriptiondetails').append(
                        ' <div class="md-card" >' +
                        '<div class="md-card-toolbar DataDiv item-div">' +
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Status == "success") {
                    ...
                    setTimeout(function () {
                        window.location = "/Masters/Item/Index"
                    }, 1000);
                } else {
    ...
    rules: {
        on_submit: [
            ...
            },
            {
                elements: "#UnitID",
                rules: [
                    { type: form.non_zero, message: "UOM is required" },
                ]
            },
            //{
