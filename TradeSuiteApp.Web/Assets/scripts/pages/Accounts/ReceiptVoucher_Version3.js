...
receipt = {
    ...
    select_aacount_head : function () {
        var self = receipt;
        var radio = $('#account-head-list tbody input[type="radio"]:checked');
    ...
    tabbed_list: function (type) {
        ...
        if ($list.length) {
            ...
            var url = "/Accounts/Receipt/GetReceiptVoucherListV3?type=" + type;
            ...
            var list_table = $list.dataTable({
                ...
                "drawCallback": function () {
                    $list.find('tbody td:not(.ReconciledDate)').on('click', function () {
                        ...
                        app.load_content("/Accounts/Receipt/DetailsV3/" + Id);
    ...
    show_reconciled_date_modal: function () {
        var self = receipt;
    ...
    bind_events: function () {
        ...
        $('body').on('click', '#btn-ok-account-head', self.select_account_head);
    ...
    save_reconciled_date: function () {
        ...
        $.ajax({
            url: '/Accounts/Receipt/SaveReconciledDate',
            data: {
            ...
            success: function (response) {
                if (response.Status == "Success") {
                    ...
                    setTimeout(function () {
                        window.location = "/Accounts/Receipt/IndexV3";
                    }, 1000);
                } else {
    ...
    save: function (IsDraft) {
        ...
        if (IsDraft) {
            data.IsDraft = true;
            url = '/Accounts/Receipt/SaveAsDraftV3';
        }
        else {
            url = '/Accounts/Receipt/SaveV3';
        }
    ...
    get_invoices: function () {
        ...
        $.ajax({
            url: '/Accounts/Receipt/GetInvoiceForReceiptVoucherV3/',
            dataType: "json",
            type: "POST",
            data: {
    ...
    validate_reconsiled_date: function () {
        var self = receipt;
        ...
        on_add: [
            {
                ...
                rules: [
                    { type: form.required, message: "Please choose AccountHead" },
                    { type: form.positive, message: 'Please select AccountHead' },
                ]
            },
            {
        ...
        on_draft: [
            {
                elements: "#AccountHeadID",
                rules: [
                    { type: form.required, message: "Please select AccountHead" },
                    { type: form.non_zero, message: "Please select AccountHead" },
                ]
            },
