DebitNote = {
    ...
    printpdf: function () {
        ...
        var id = $("#ID").val();
        $.ajax({
            url: '/Reports/Accounts/DebitNotePrintPdf',
            data: {
                id: $("#ID").val(),
            },
            dataType: "json",
    ...
    on_save: function () {
        ...
        var location = "/Accounts/DebitNote/Index";
        var url = '/Accounts/DebitNote/Save';

        if ($(this).hasClass("btnSavedraft")) {
            data.IsDraft = true;
            url = '/Accounts/DebitNote/SaveAsDraft'
            self.error_count = self.validate_form();
        } else {
            if ($(this).hasClass("btnSaveNew")) {
                location = "/Accounts/DebitNote/Create";
            }
            self.error_count = self.validate_form();
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Status == "Success") {
                    app.show_notice("DebitNote created successfully");
                    setTimeout(function () {
    ...
    list: function () {
        var self = DebitNote;
        $('#tabs-DebitNote').on('change.uk.tab', function (event, active_item, previous_item) {
    ...
    tabbed_list: function (type) {
        ...
        if ($list.length) {
            ...
            var url = "/Accounts/DebitNote/GetDebitNoteList?type=" + type;
            var list_table = $list.dataTable({
                ...
                "drawCallback": function () {
                    $list.find('tbody td:not(.ID)').on('click', function () {
                        ...
                        app.load_content("/Accounts/DebitNote/Detail/" + Id);
